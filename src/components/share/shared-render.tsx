  ...
  return (
    <main className="mx-auto flex min-h-dvh w-full max-w-2xl flex-col gap-8 px-4 py-10 sm:px-6 sm:py-14">
      <header className="space-y-1">
        <p className="text-muted-foreground text-xs font-medium tracking-wide uppercase">Fitcheck</p>
        <h1 className="text-2xl font-semibold tracking-tight text-balance sm:text-3xl">{shared.outfitName}</h1>
        <p className="text-muted-foreground text-sm">{formatDate(shared.createdAt)}</p>
      </header>
      ...
      <ItemImage
        ...
        aspect="aspect-[3/4]"
        priority
        className="ring-foreground/10 w-full rounded-2xl ring-1"
      />
      ...
      <footer className="text-muted-foreground mt-auto border-t pt-6 text-sm">
        Made with{" "}
        <Link href={routes.home} className="text-foreground font-medium underline underline-offset-4">
          Fitcheck
        </Link>{" "}
