    ...
    const results = await Promise.all(
      targets.map(async ({ renderId, stepKey }) => {
        try {
          ...
          // The render row and its step were already marked failed; onRenderComplete refunds.
          return false;
        }
      }),
    );
  ...
  handler: async (ctx, args): Promise<null> => {
    ...
    for (const render of unfinished) {
      if (render.status === "pending") {
        await markRenderFailed(ctx, render._id, workflowError ?? "This image didn't finish.");
      }
    }
    ...
    if (unused > 0) await refund(ctx, job, unused, "Render failed");

    const firstError = renders.find((render) => render.status === "failed")?.error;
    ...
    if (job.steps.some((step) => step.key === RENDER_STEPS.finalize)) {
      await setStep(ctx, job._id, RENDER_STEPS.finalize, {
        status: "done",
        meta: { rendered: done.length, failed: unfinished.length, refunded: unused },
      });
    }
