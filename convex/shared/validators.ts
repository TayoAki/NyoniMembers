...
export function literals<const T extends readonly string[]>(values: T): LiteralUnion<T> {
  const members = values.map((value) => v.literal(value)) as VLiteral<T[number]>[];
