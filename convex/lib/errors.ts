...
export function appError(code: ErrorCode, message: string, details?: AppErrorDetails): ConvexError<AppErrorData> {
  return new ConvexError<AppErrorData>({ code, message, ...(details ? { details } : {}) });
}
